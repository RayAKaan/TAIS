"""
Metacognitive engine for TAIS Swarm V6.

Replaces V5.5's hardcoded heuristics with:
- Prediction error tracking per strategy
- Strategy selection based on error history
- Self-model of learning speed, memory reliability, social bias,
  exploration tendency, and planning depth
"""
...
class PredictionTracker:
    ...
    def to_dict(self) -> dict:
        return {s: {"accuracy": round(self.accuracy(s), 3), "samples": len(q)}
...
class SelfModel:
    ...
    def update_from_accuracy(self, accuracy: float, confidence: float):
        ...
        self.memory_reliability = min(1.0, max(0.1, self.memory_reliability + lr * (accuracy - 0.5) * 2))
        self.exploration_tendency = max(0.05, min(0.8, self.exploration_tendency + lr * (0.5 - accuracy)))
...
class MetacognitiveEngine:
    ...
    def select_strategy(self, tick: int, strategies: List[str], urgency: float = 0.5) -> str:
        ...
        if random.random() < self.self_model.exploration_tendency:
    ...
    def record_outcome(self, strategy: str, prediction: Any, outcome: Any, correct: bool, tick: int):
        self.predictions.record(strategy, prediction, outcome, correct)
        if self.self_model.prediction_count > 0 and self.self_model.prediction_count % 10 == 0:
