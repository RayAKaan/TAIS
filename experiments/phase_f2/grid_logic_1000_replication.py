#!/usr/bin/env python3
"""Phase F2 - Experiment 4: 1000-Seed Grid->Logic Replication.

Replicates the Phase 5 logic transfer experiment at higher statistical
power (1000 seeds) to confirm d ~ -0.57 and the prediction paradox fix.
"""
...
def format_table(results, seeds, pretrain_ticks, eval_ticks):
    ...
    lines = []
    lines.append("=" * 122)
    lines.append("  PHASE F2 - Experiment 4: 1000-Seed Grid->Logic Replication")
    lines.append("=" * 122)
...
def main():
    p = argparse.ArgumentParser(description="Phase F2 - 1000-Seed Grid->Logic Replication")
    ...
    p.add_argument("--output", type=str, default="results/phase_f2/grid_logic_1000_replication.txt")
    p.add_argument("--verbose", action="store_true")
    args = p.parse_args()

    # If 1000 seeds would be too slow, recommend 200 as preliminary.
    if args.seeds >= 1000:
        print("Note: 1000 seeds may take 60-90s. Use --seeds 200 for a quicker run.", file=sys.stderr)

    print(f"Grid->Logic Replication: seeds={args.seeds}, pretrain={args.pretrain}, eval={args.eval}")
    t0 = time.time()
