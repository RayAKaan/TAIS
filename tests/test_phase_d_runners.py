...
class TestPhaseDImports(unittest.TestCase):
    def test_import_composition(self):
        import experiments.phase_d.composition
        self.assertTrue(hasattr(experiments.phase_d.composition, "build_suite"))

    def test_import_scaling_law(self):
        import experiments.phase_d.scaling_law
        self.assertTrue(hasattr(experiments.phase_d.scaling_law, "build_suite_domain_count"))

    def test_import_reverse_transfer(self):
        import experiments.phase_d.reverse_transfer
        self.assertTrue(hasattr(experiments.phase_d.reverse_transfer, "run_trial"))

    def test_import_curriculum(self):
        import experiments.phase_d.curriculum
        self.assertTrue(hasattr(experiments.phase_d.curriculum, "build_suite"))

    def test_import_cognitive_contribution(self):
        import experiments.phase_d.cognitive_contribution
        self.assertTrue(hasattr(experiments.phase_d.cognitive_contribution, "build_suite"))


class TestPhaseDSmoke(unittest.TestCase):
    def test_composition_smoke(self):
        from experiments.phase_d.composition import build_suite
    ...
    def test_scaling_domain_count_smoke(self):
        from experiments.phase_d.scaling_law import build_suite_domain_count
        suite = build_suite_domain_count(seeds=2, eval_ticks=5, pretrain_ticks=2)
        results = suite.run()
        self.assertIn("fresh", results.records)

    def test_scaling_horizon_smoke(self):
        from experiments.phase_d.scaling_law import build_suite_horizon
    ...
    def test_curriculum_smoke(self):
        from experiments.phase_d.curriculum import build_suite
        suite = build_suite(seeds=2, eval_ticks=5, pretrain_ticks=2)
        results = suite.run()
        self.assertIn("fresh", results.records)
    ...
    def test_cognitive_contribution_smoke(self):
        from experiments.phase_d.cognitive_contribution import build_suite
        suite = build_suite(seeds=2, eval_ticks=5, pretrain_ticks=2)
        results = suite.run()
        self.assertIn("fresh", results.records)
    ...
    def test_reverse_transfer_trial(self):
        from experiments.phase_d.reverse_transfer import run_trial
...
class TestPhaseDOutput(unittest.TestCase):
    ...
    def test_reverse_transfer_output(self):
        from experiments.phase_d.reverse_transfer import main as rt_main
        import sys
