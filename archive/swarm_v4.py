"""
TAIS-LANG v4: World · Memory · Reference
=========================================

No LLM. No pretrained language model. No codebook sentence generation.

v4 expands v3 from a small food/danger arena into an ecological world where
speech becomes useful because agents must talk about absent things:
...
@app.route("/health")
def health():
    return jsonify({"ok": True, "version": "v4", "tick": swarm.tick, "population": len([m for m in swarm.motes if m.is_alive()])})
...
def reset_route():
    ...
    start_thread()
    return jsonify({"ok": True, "version": "v4"})
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TAIS-LANG v4 World · Memory · Reference")
...
if __name__ == "__main__":
    ...
    if args.headless:
        ...
        if args.load:
            ...
            swarm = Swarm(world_size=args.world, population=args.population)
        start_thread()
        print("TAIS-LANG v4: World · Memory · Reference")
